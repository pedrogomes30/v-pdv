<template>
    <v-navigation-drawer v-model="drawer" app color="white" mini-variant mini-variant-width="100">
        <v-avatar class="d-block text-center mx-auto mt-4" size="56">
            <v-img src="../assets/defaultUser.jpg" /> 
            <v-badge bordered overlap color="red" dot></v-badge>
        </v-avatar>
        ...
        >
            ...
            <br>
            <span class="caption">Deslogar</span>    
        </div>
    </v-navigation-drawer>
</template>
...
export default {
    data:()=>({
        ...
        drawer:null,
        items: [
            {icon:'fas fa-newspaper',text:'Novidades'},
            {icon:'fas fa-cart-plus',text:'Caixa'},
            {icon:'fas fa-history',text:'Histórico'},
            {icon:'mdi-cart-check',text:'Fechamento Caixa'},
            {icon:'fas fa-users',text:'Clientes'},
            {icon:'fas fa-percent',text:'Promoções'},
...
.list-item-group .v-list-item--active {
